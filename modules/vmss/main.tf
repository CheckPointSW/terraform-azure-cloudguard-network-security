...
// Standard deployment
resource "azurerm_lb_rule" "lbnatrule_standard" {
  depends_on = [
    azurerm_lb.frontend_lb[0], azurerm_lb_probe.azure_lb_healprob, azurerm_lb.backend_lb[0]
  ]
  ...
  backend_address_pool_ids       = count.index == 0 ? [azurerm_lb_backend_address_pool.frontend_lb_pool[0].id] : [azurerm_lb_backend_address_pool.backend_lb_pool[0].id]
...
resource "azurerm_lb_rule" "lbnatrule_external" {
  depends_on = [
    azurerm_lb.frontend_lb[0], azurerm_lb_probe.azure_lb_healprob
  ]
  count                          = var.deployment_mode == "External" ? 1 : 0
  loadbalancer_id                = azurerm_lb.frontend_lb[0].id
  name                           = "${var.vmss_name}-app-1"
  ...
  backend_address_pool_ids       = [azurerm_lb_backend_address_pool.frontend_lb_pool[0].id]
...
resource "azurerm_lb_rule" "lbnatrule_internal" {
  depends_on = [
    azurerm_lb_probe.azure_lb_healprob, azurerm_lb.backend_lb[0]
  ]
  count                          = var.deployment_mode == "Internal" ? 1 : 0
  loadbalancer_id                = azurerm_lb.backend_lb[0].id
  name                           = "backend-lb"
  ...
  frontend_port                  = "0"
  backend_port                   = "0"
  backend_address_pool_ids       = [azurerm_lb_backend_address_pool.backend_lb_pool[0].id]
